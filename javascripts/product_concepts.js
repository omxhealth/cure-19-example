...
var api_host = "https://www.example-db.com/v1/";
...
$(document).ready(function() {
  ...
  $(".drug_autocomplete").select2({
    ...
    ajax: {
      url: api_host + encodeURI("us/product_concepts"),
      delay: 100,
      ...
      success: function(data) {
        var url = encodeURI(api_host + "us/product_concepts?q=" + $(".select2-search__field").val());
  ...
  $("#product-concepts-tutorial .drug_autocomplete").on("change", function(e) {
    ...
    if ($(this).val()) {
      var url = api_host + encodeURI("us/product_concepts/" + $(this).val() + "/routes")
      $.ajax({
        url: url,
        delay: 100,
        success: function(data) {
  ...
  $("#product-concepts-tutorial .route_autocomplete").on("change", function(e) {
    ...
    clearTableResults(products_table);
    if ($(this).val()) {
      var url = api_host + encodeURI("us/product_concepts/" + $(this).val() + "/strengths")
      $.ajax({
        url: url,
        delay: 100,
        success: function(data) {
  ...
  $("#product-concepts-tutorial .strength_autocomplete").on("change", function(e) {
    ...
    if ($(this).val()) {
      $("#loader").show();
      var url = api_host + encodeURI("us/product_concepts/" + $(this).val() + "/products")
      $.ajax({
        url: url,
        delay: 100,
        success: function(data) {
